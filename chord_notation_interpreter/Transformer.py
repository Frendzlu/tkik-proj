...
class EvalExpressions(Transformer):
	...
	def chord_base_size(self, args):
		if int(args[0]) not in [7, 9, 11, 13]:
			self.error_stack.append(f"Line {args[0].line}, column {args[0].column}: Chord size of {args[0]} is not supported. Allowed values are 7, 9, 11, 13.")
	...
	def preamble(self, args):
		if int(args[1]) in self.used_segno_numbers.keys():
			self.error_stack.append(f"Line {args[1].line}, column {args[1].column}: segno numbers conflict - "
			                        f"previously defined in line {self.used_segno_numbers[
			                        int(args[1])][0]}, column "
